...
class EventTest < ActiveSupport::TestCase
  ...
  test "create with nonexistant account-id should fail" do
    ...
    assert_no_difference "Event.count" do
      assert_raise(ActiveRecord::RecordNotFound) do
        subscriptions(:john).events.create(@event_base.merge(user: users(:john)))
      end
    end
  end
  ...
  test "create without actor should not pass validation" do
    ...
    assert_no_difference "Event.count" do
      ...
      assert event.errors[:actor_name].any?
    end
  end
  ...
  test "create with unrecognized event role should fail validation" do
    ...
    assert_no_difference "Event.count" do
      event = subscriptions(:john).events.create(@event_base.merge(user: users(:john)))
      assert event.errors[:line_item].any?
    end
  end
  ...
  test "create with tagged_items should generate tagged_items for event" do
    event = subscriptions(:john).events.create(@event_base.merge(
        tagged_items: [ { tag_id: tags(:john_lunch).id, amount: 500 } ], user: users(:john)))
  ...
  test "create with named tagged_items should find or create tagged_items" do
    event = subscriptions(:john).events.create(@event_base.merge(
        tagged_items: [ { tag_id: "n:milk", amount: 500 },
                           { tag_id: "n:fruit", amount: 750 } ], user: users(:john)))
